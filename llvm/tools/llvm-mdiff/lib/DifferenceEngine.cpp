...
void DifferenceEngine::diff(const Module *L, const Module *R) {
  ...
  SmallVector<const Function* , 20> LeftFuncs;
  SmallVector<const Function* , 20> RightFuncs;
  
  for (Module::const_iterator I = L->begin(), E = L->end(); I != E; ++I) {
    const Function *LFn = &*I;
    if(!LFn->empty()) {
      LeftFuncs.push_back(LFn);
    }
  }
  ...
  for (Module::const_iterator I = R->begin(), E = R->end(); I != E; ++I) {
    ...
    if(!RFn->empty()) {
      RightFuncs.push_back(RFn);
    }
  }
  // For exebench, we ignore the difference between function names
  if (LeftFuncs.size() != RightFuncs.size()) {
    logf("left module has different number of functions than right module");
        // << LeftFuncs.size() << RightFuncs.size();
    return;
  }
  for (unsigned i = 0; i < LeftFuncs.size(); i++) {
    const Function *LFn = LeftFuncs[i];
    const Function *RFn = RightFuncs[i];
...
bool DifferenceEngine::equivalentAsOperands(const GlobalValue *L,
  ...
  return L->getValueType() == R->getValueType();
}
